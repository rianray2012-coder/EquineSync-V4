"""Stripe live-payment Wave B/C readiness tests.

Wave B/C is read-only. These tests pin redacted config, catalog, and origin
proof behavior without calling Stripe, creating sessions, replaying webhooks,
or writing data.
"""
...
def test_ready_live_config_passes_without_enabling_live_money():
    ...
    assert report["allowed_origins"] == {
        "configured": True,
        "count": 1,
        "scheme_counts": {"https": 1},
        "all_https": True,
    }
    assert report["catalog"] == {
        "approved_product_count": 10,
        "approved_self_service_plan_count": 8,
        "approved_price_count": 16,
        "approved_addon_count": 12,
        "env_override_mismatch_count": 0,
    }
    assert report["return_path_contract"] == {
...
def test_live_config_report_blocks_incomplete_or_enabled_live_session_state():
    env = {
        ...
        "STRIPE_PRICE_STARTER_BARN_MONTHLY": "price_stale",
    }
    ...
    assert "stripe_allowed_origin_not_https" in kinds
...
def test_rendered_wave_b_report_never_contains_raw_secret_inputs():
    ...
    assert "# Stripe Live-Payment Readiness Wave B/C" in markdown
...
def test_wave_c_route_source_uses_locked_checkout_and_portal_return_paths():
    route_src = (ROOT / "backend" / "routes" / "subscriptions.py").read_text()

    assert '/billing/success?session_id={{CHECKOUT_SESSION_ID}}' in route_src
    assert '/billing/subscription?cancelled=1' in route_src
    assert 'return_url": f"{normalized_origin}/billing/subscription"' in route_src
