...
from core.app_store_readiness_proof import (  # noqa: E402
    build_app_store_readiness_proof,
    render_app_store_readiness_report,
)


DEFAULT_OUTPUT = ROOT / "outputs" / "bn18e_app_store_readiness_report.md"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build-Next-18E read-only App Store / Google Play readiness proof report."
    )
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT), help="Markdown report output path.")
    parser.add_argument(
        "--fail-on-blockers",
        action="store_true",
        help="Exit non-zero if app-store readiness blockers are present.",
    )
    ...
    print(f"Build-Next-18E app-store readiness proof written: {output}")
    print(
        "Summary: "
