...
from core.rf8_staff_workforce_model import (  # noqa: E402
    ROOT,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the RF8 staff workforce model report.")
    parser.add_argument(
        "--output",
        default="outputs/rf8_staff_workforce_model_report.md",
    ...
    output_path.write_text(render_rf8_report(report), encoding="utf-8")

    blocked = report["status_counts"].get("blocked", 0) + report["status_counts"].get("missing", 0)
    print(f"RF8 report written: {output_path}")
