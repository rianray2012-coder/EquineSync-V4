                  ...
                  return (
                    <div key={record.id} className={`rounded-xl border border-equine-hairline bg-equine-soft/55 p-4 ${savingId === record.id ? "opacity-60" : ""}`} data-testid={`health-reminder-card-${record.id}`}>
                      ...
                      <div className="hairline mt-3 pt-3 flex items-center justify-between gap-2">
                        {status !== "complete" ? (
                          <button type="button" onClick={() => setStatus(record, "complete")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1" data-testid={`health-reminder-complete-${record.id}`}>
                            <CheckCircle2 className="w-3.5 h-3.5" /> Mark complete
                          </button>
                        ) : (
                          <button type="button" onClick={() => setStatus(record, "scheduled")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft">
                            Reopen
                          </button>
                        )}
