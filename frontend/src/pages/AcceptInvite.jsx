...
export default function AcceptInvite() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success("Welcome to EquineSync");
      ...
      <div className="relative hidden lg:block bg-equine-navy">
        ...
        <div className="relative z-10 h-full flex flex-col justify-between p-12 text-white">
          ...
          <div className="max-w-md">
            <div className="text-[10.5px] tracking-[0.28em] uppercase text-brand-lavender font-semibold mb-5">You've been invited</div>
            <h2 className="font-display text-5xl xl:text-6xl leading-[1.05] text-white">
              Your EquineSync workspace is ready.
            </h2>
            <p className="mt-6 text-equine-silver/85 text-[15px] leading-relaxed">
              Set your password to join the right workspace and start with the tools your role needs.
            </p>
          </div>
          ...
          {!error && invite && (
            <>
              ...
              <p className="mt-3 text-equine-silver/70">
                You've been invited as <span className="text-brand-lilac capitalize">{invite.role.replace('_', ' ')}</span> by <span className="text-equine-ivory">{invite.invited_by_name}</span>.
              </p>
              ...
              <form onSubmit={submit} className="mt-8 space-y-5" data-testid="accept-form">
                ...
                <div>
                  ...
                  <input value={fullName} onChange={(e) => setFullName(e.target.value)} required data-testid="accept-name"
                    className="w-full bg-equine-soft border border-equine-graphite/60 rounded-xl px-4 py-3 text-equine-ivory focus:outline-none focus:border-brand-lilac" />
                </div>
                <div>
                ...
                <button disabled={submitting} data-testid="accept-submit" className="btn-primary w-full disabled:opacity-60 inline-flex items-center justify-center gap-2">
                  <ShieldCheck className="w-4 h-4" /> {submitting ? "Setting up…" : "Accept invitation"}
                </button>
