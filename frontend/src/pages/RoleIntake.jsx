...
const emptyCompletion = { percent: 0, missing_fields: [] };

const isMissingIntakeProfile = (error) => error?.response?.status === 404;

const TODAY_PULSE_SCHEMA = "bn15a.today_pulse.v1";
...
function useTodayPulse() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        if (!alive) return;
        setPulse(r.data?.schema_version === TODAY_PULSE_SCHEMA ? r.data : null);
      })
      .catch(() => {
...
function visiblePulseCard(pulse, key) {
  const card = pulse?.cards?.[key];
  return pulse?.schema_version === TODAY_PULSE_SCHEMA && card?.visible ? card : null;
}

function pluralCount(value, label) {
  const count = Number(value || 0);
  return `${count} ${label}${count === 1 ? "" : "s"}`;
}

function pulsePanelText(pulse, key, fallback) {
  const card = visiblePulseCard(pulse, key);
  if (!card) return fallback;

  if (key === "todays_work") {
    const count = Number(card.count || 0);
    const pending = Number(card.pending || 0);
    const completed = Number(card.completed || 0);
    return count
      ? `Today: ${pluralCount(count, "task item")}, ${pending} pending, ${completed} completed.`
      : "No task items due today.";
  }

  if (key === "horse_care") {
    const horses = Number(card.horses || 0);
    const openAlerts = Number(card.open_alerts || 0);
    const urgentAlerts = Number(card.urgent_alerts || 0);
    if (!horses && !openAlerts && !urgentAlerts) return fallback;
    const alertText = openAlerts
      ? `, ${openAlerts} open alert${openAlerts === 1 ? "" : "s"}${urgentAlerts ? `, ${urgentAlerts} urgent` : ""}`
      : "";
    return `Horse care: ${pluralCount(horses, "horse")} in your current scope${alertText}.`;
  }

  if (key === "owner_requests") {
    const pending = Number(card.pending || card.count || 0);
    return pending ? `Owner requests: ${pending} open.` : "No open owner requests.";
  }

  if (key === "setup") {
    const percent = Number(card.completion_percent);
    return Number.isFinite(percent) ? `Setup: ${percent}% complete.` : fallback;
  }

  if (key === "plan_usage") {
    const active = Number(card.active_horses || 0);
    const limit = card.horse_limit;
    if (limit === null || limit === undefined) {
      return `Plan usage: ${pluralCount(active, "active horse")}.`;
    }
    return `Plan usage: ${active} active horses of ${limit} included.`;
  }
    ...
    <div className="max-w-6xl mx-auto pb-20 lg:pb-8" data-testid="role-home-staff">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
        <section className="rounded-2xl bg-equine-card border border-equine-hairline p-5" data-testid="staff-intake-shell">
          <div className="flex items-start justify-between gap-4 mb-5">
            ...
            <div className="w-20 h-20 rounded-2xl bg-equine-navy/60 border border-white/10 flex items-center justify-center">
              <span className="font-display text-3xl text-equine-icyLight">{completion.percent || 0}%</span>
            </div>
          </div>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl border text-[12.5px] transition ${
                          active
                            ? "bg-equine-icyLight text-equine-navy border-equine-icyLight"
                            : "bg-white/[0.03] text-equine-platinum/75 border-white/10 hover:border-equine-icyLight/50"
                        }`}
                      >
                        {label}
                      </button>
        ...
        <aside className="space-y-5">
          {[
            ["Today's Work", pulsePanelText(pulse, "todays_work", "Task assignment and completion stay in the approved task workflow."), ClipboardList],
            ["Assigned Horses", pulsePanelText(pulse, "horse_care", "Horse access is not granted from this page."), Heart],
            ...
            <section key={title} className="rounded-2xl bg-equine-card border border-equine-hairline p-5" data-testid={`staff-panel-${title.toLowerCase().replace(/[^a-z]+/g, "-")}`}>
              <div className="w-10 h-10 rounded-xl bg-equine-navy/70 flex items-center justify-center mb-4">
                <Icon className="w-5 h-5 text-equine-icyLight" strokeWidth={1.5} />
              </div>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 lg:pb-8" data-testid="role-home-manager">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
        ...
        <aside className="space-y-5">
          {[
            ["Today's Work", pulsePanelText(pulse, "todays_work", "Task creation and assignment stay in the approved task workflow."), ClipboardList],
            ["Team Coordination", "Staff invites, roles, and permissions are not changed from this page.", ShieldCheck],
            ["Horse Care Oversight", pulsePanelText(pulse, "horse_care", "HorseOps records are not created until the horse workflow is used."), Heart],
            ["Facility Tasks", "Facility setup and configuration remain separate from this intake.", CalendarDays],
            ["Owner Requests", pulsePanelText(pulse, "owner_requests", "Owner communication and request workflows stay in their approved surfaces."), MessageSquare],
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 lg:pb-8" data-testid="role-home-barn-owner">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
        ...
        <aside className="space-y-5">
          {[
            ["Facility Setup", pulsePanelText(pulse, "setup", "Formal setup remains separate. This intake only captures founder intent."), ShieldCheck],
            ["Staff & Roles", "Staff invites and role assignments are not created from this page.", ClipboardList],
            ["Horses", pulsePanelText(pulse, "horse_care", "Horse records are not created here; this page only records expected facility scale."), Heart],
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
        ...
        <aside className="space-y-5">
          <section className="rounded-2xl bg-equine-card border border-equine-hairline p-5" data-testid="owner-panel-my-horse">
            <div className="w-10 h-10 rounded-xl bg-equine-navy/70 flex items-center justify-center mb-4">
              <Heart className="w-5 h-5 text-equine-icyLight" strokeWidth={1.5} />
            </div>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 lg:pb-8" data-testid="role-home-rider">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
        ...
        <aside className="space-y-5">
          {[
            ["Schedule", "Your barn or trainer will publish lesson and ride times here after scheduling is connected.", CalendarDays],
            ["Lessons", "Lesson plans, prep notes, and trainer feedback will appear here after the program is connected.", ClipboardList],
            ["Horse Context", pulsePanelText(pulse, "horse_care", "Barn-wide horse count is hidden unless your barn enables community count."), Heart],
            ...
            ["Documents", "Waivers, policies, and signed forms will appear here after approved document workflows are connected.", FileText],
          ].map(([title, body, Icon]) => (
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-5">
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4 text-[13px] text-equine-platinum/75 leading-relaxed" data-testid="guardian-consent-status">
                Consent status: <span className="text-equine-icyLight">pending formal consent</span>. This intake does not replace signed waivers, legal guardian approval, or barn participation requirements.
              </div>
        ...
        <aside className="space-y-5">
          {[
            ...
            ["Schedule", "Lesson and ride times will appear here once the barn connects this rider.", CalendarDays],
            ["Horse Context", pulsePanelText(pulse, "horse_care", "Barn-wide horse count is hidden unless your barn enables community count."), Heart],
            ["Progress Notes", "Barn-approved progress notes and trainer summaries will appear here after the program is connected.", MessageSquare],
            ["Documents", "Waivers, barn policies, and signed forms will appear here after approved document workflows are connected.", FileText],
            ["Requests", "Guardian requests will stay separate from staff-only workflows.", ClipboardList],
...
export default function RoleIntake({ forcedProfile }) {
  const { profile: routeProfile } = useParams();
  const profile = forcedProfile || routeProfile;
  const { user } = useAuth();
