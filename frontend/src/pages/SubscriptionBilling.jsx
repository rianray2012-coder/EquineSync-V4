...
// palette: lavender, ivory, icy blue, charcoal, and muted status colors only.
              ...
              {SUBSCRIBABLE_TIERS.has(planTier) && !resumePending && (
                <>
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-full text-[12.5px] tracking-wide font-medium border border-equine-lavenderDeep/50 text-equine-lavenderDeep hover:bg-equine-lavenderSoft/15 disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center gap-1.5"
        ...
        <Card className="bg-equine-card">
          ...
          <ul className="space-y-3 text-[13px] text-equine-inkMuted leading-relaxed">
            <li className="flex items-start gap-2">
              <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 text-equine-lavenderDeep flex-shrink-0" />
            ...
            <li className="flex items-start gap-2">
              <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 text-equine-lavenderDeep flex-shrink-0" />
              <span>Barn invoices, physical payment tracking, and trainer charges are tracked separately on the <Link to="/billing" className="underline text-equine-lavenderDeep hover:text-equine-ink">Billing</Link> page.</span>
            </li>
          </ul>
        </Card>
      </div>
...
function UsageMeter({ label, used, limit, testid }) {
  ...
  if (hasLimit) {
    if (used >= limit) {
      ...
      toneBar = "bg-equine-lavenderSoft";
      tonePill = "info";
    }
  } else {
    toneBar = "bg-equine-lavenderSoft";
...
function PlanPickerCard({ plan, currentTier, currentCycle, onCheckout, busyAction }) {
  ...
  return (
    <Card
      hover={!isCurrent}
      className={isCurrent ? "border-equine-lavenderDeep/50" : ""}
      ...
      {!contactSales && (plan.has_monthly || plan.has_annual) && (
        <div className="inline-flex bg-equine-soft rounded-full p-0.5 mb-4" data-testid={`plan-cycle-toggle-${plan.tier_code}`}>
          ...
          >
            Annual
            {savings != null && (
              <span className="text-[10px] tracking-wider uppercase text-equine-lavenderDeep font-medium" data-testid={`plan-savings-${plan.tier_code}`}>
                Save {savings}%
              </span>
            )}
          </button>
        ...
        <ul className="space-y-1.5 mb-5 text-[12.5px] text-equine-inkMuted">
          {plan.feature_limits.horses != null && (
            <li className="flex items-center gap-2">
              <Sparkles className="w-3 h-3 text-equine-lavenderDeep" />
      ...
      <button
        ...
        className={`w-full text-[12.5px] tracking-wide font-medium py-2.5 rounded-full transition-colors inline-flex items-center justify-center gap-1.5 ${
          isCurrentCycle
            ...
            ? "border border-equine-lavenderDeep/50 text-equine-lavenderDeep hover:bg-equine-lavenderSoft/15"
