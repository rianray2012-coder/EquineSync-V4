                ...
                {grouped[status].map((record) => {
                  ...
                  return (
                    <div key={record.id} className={`rounded-xl border border-equine-hairline bg-equine-soft/55 p-4 ${savingId === record.id ? "opacity-60" : ""}`} data-testid={`emergency-workflow-card-${record.id}`}>
                      ...
                      {contact.phone && (
                        <a href={`tel:${contact.phone}`} className="mt-3 inline-flex items-center gap-1 text-[12px] text-equine-navy hover:text-equine-lilac">
                      ...
                      <div className="hairline mt-3 pt-3 flex flex-wrap items-center gap-2">
                        {data.owner_status !== "authorized" && (
                          <button type="button" onClick={() => updateRecord(record, { owner_status: "authorized" })} className="text-[12px] text-equine-navy hover:text-equine-lilac">
                            Owner authorized
                          </button>
                        )}
                        {data.vet_status !== "on_site" && data.vet_status !== "complete" && (
                          <button type="button" onClick={() => updateRecord(record, { vet_status: "on_site" })} className="text-[12px] text-equine-navy hover:text-equine-lilac">
                            Vet on site
                          </button>
                        )}
                        {status !== "resolved" && (
                          <button type="button" onClick={() => updateRecord(record, { workflow_status: "resolved", vet_status: "complete" })} className="text-[12px] text-equine-navy hover:text-equine-lilac inline-flex items-center gap-1">
