                    ...
                    <div key={record.id} className={`rounded-xl border border-equine-hairline bg-equine-soft/55 p-4 ${savingId === record.id ? "opacity-60" : ""}`} data-testid={`training-plan-card-${record.id}`}>
                      ...
                      {canWriteTrainingPlan && (
                        <div className="hairline mt-3 pt-3 flex flex-wrap items-center gap-2">
                          {status !== "achieved" && (
                            <button type="button" onClick={() => setStatus(record, "achieved")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1">
                              <CheckCircle2 className="w-3.5 h-3.5" /> Achieved
                            </button>
                          )}
                          {status !== "paused" && (
                            <button type="button" onClick={() => setStatus(record, "paused")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1">
                              <PauseCircle className="w-3.5 h-3.5" /> Pause
                            </button>
                          )}
                          {status !== "active" && (
                            <button type="button" onClick={() => setStatus(record, "active")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft">
                              Activate
                            </button>
                          )}
