                  ...
                  return (
                    <div key={record.id} className={`rounded-xl border border-equine-hairline bg-equine-soft/55 p-4 ${savingId === record.id ? "opacity-60" : ""}`} data-testid={`staff-task-card-${record.id}`}>
                      ...
                      <div className="hairline mt-3 pt-3 flex flex-wrap items-center gap-2">
                        {status !== "in_progress" && (
                          <button type="button" onClick={() => setStatus(record, "in_progress")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1">
                            <PlayCircle className="w-3.5 h-3.5" /> Start
                          </button>
                        )}
                        {status !== "complete" && (
                          <button type="button" onClick={() => setStatus(record, "complete")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1">
                            <CheckCircle2 className="w-3.5 h-3.5" /> Complete
                          </button>
                        )}
                        {status !== "blocked" && (
                          <button type="button" onClick={() => setStatus(record, "blocked")} className="text-[12px] text-equine-navy hover:text-equine-lavenderSoft inline-flex items-center gap-1">
