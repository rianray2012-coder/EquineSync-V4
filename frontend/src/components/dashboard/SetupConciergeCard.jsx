  ...
  return (
    <Card elevated hover={false} className="mb-10 animate-fade-in" data-testid="setup-checklist-card">
      <div className="flex items-start gap-4 mb-5">
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-equine-lilac to-equine-lilac flex items-center justify-center flex-shrink-0 shadow-lg">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-3 flex items-center gap-4">
            <div className="flex-1 h-1.5 bg-equine-soft rounded-full overflow-hidden max-w-md">
              <div
                className="h-full bg-gradient-to-r from-equine-lilac to-equine-lavender transition-all duration-500"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
        {steps.map((s) => {
          ...
          const tone = isDone ? "border-equine-sage/40 bg-equine-sage/8"
            : isInProgress ? "border-equine-lilac/40 bg-equine-lilac/8"
          ...
          return (
            <Link
              ...
              className={`px-3 py-2.5 rounded-xl border transition-all duration-200 hover:border-equine-lilac hover:bg-equine-elevated flex items-center gap-2.5 tap-44 ${tone}`}
            >
              {isDone
                ...
                : isSkipped
                  ...
                  : <Ic strokeWidth={1.5} className={`w-4 h-4 flex-shrink-0 ${isInProgress ? "text-equine-lavender" : "text-equine-platinum/70"}`} />
              }
              <div className="min-w-0 flex-1">
                <div className="text-[12.5px] text-equine-ivory truncate">
                  {STEP_META[s.id]?.label || s.label}
                </div>
                <div className="text-[9.5px] uppercase tracking-[0.18em] text-equine-lilac/65">
                  {status.replace("_", " ")}
                </div>
              </div>
            </Link>
