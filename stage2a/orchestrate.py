...
def _read_record(pid_file: Path) -> dict[str, object] | None:
    try:
        ...
        if value.get("owner") != "ES-PKG-2026-004-V003" or not isinstance(value.get("pid"), int):
            return None
        return value
...
def _write_record(pid_file: Path, pid: int, kind: str, command: list[str]) -> None:
...
def _expected(record: dict[str, object] | None, kind: str) -> bool:
    ...
    if not files:
        return False
    if kind == "mongo":
        return "/mongod" in files
    expected_cwds = (str(ROOT), str(REPO / "backend"))
    return "python" in files and any(cwd in files for cwd in expected_cwds)
...
def _terminate(kind: str, pid_file: Path, port: int) -> dict[str, object]:
    ...
    pid_file.unlink(missing_ok=True)
    return {"kind": kind, "pid": pid, "status": "STOPPED", "forced": forced, "port_closed": True}
...
def start(profile: str = "full", failpoint: str | None = None) -> dict[str, object]:
    ...
    mongo = subprocess.Popen(
        ["mongod", "--dbpath", str(RUNTIME / "mongo"), "--port", str(MONGO_PORT), "--bind_ip", "127.0.0.1", "--nounixsocket", "--quiet"],
        stdout=mongo_log,
        stderr=subprocess.STDOUT,
        env=env,
        start_new_session=True,
    )
    _write_record(MONGO_PID, mongo.pid, "mongo", ["mongod", "--dbpath", str(RUNTIME / "mongo"), "--port", str(MONGO_PORT)])
    try:
        ...
        _write_record(API_PID, api.pid, "api", command)
