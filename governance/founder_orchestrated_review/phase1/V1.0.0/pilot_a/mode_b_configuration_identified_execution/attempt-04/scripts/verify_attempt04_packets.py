...
EXPECTED_COMMIT = "34c427c0196d3f8273ac3ea88ad05a2bbe5a2c29"
EXPECTED_ORACLE_SHA256 = "4a862de152e7ba348c5250dc714aa2441f61b786b318112fc3e010de6f201de8"
EXPECTED_STATE = os.environ.get("A04_PACKET_STATE", "FROZEN")

ROLES = {
    "ES-RA-02": (
        "ES-RA-02_REVIEW_EXECUTION_PROFILE_V1.0.0.json",
        "ES-RA-02_SEGREGATED_REVIEW_AGENT.md",
        "ES-PH1-MB04-CANARY-ESRA02-4a91d8c04c6b427e",
    ),
    "ES-RA-03": (
        "ES-RA-03_REVIEW_EXECUTION_PROFILE_V1.0.0.json",
        "ES-RA-03_ADVERSARIAL_CHALLENGE_AGENT.md",
        "ES-PH1-MB04-CANARY-ESRA03-c703fa8c9b2945da",
    ),
    "ES-RA-04": (
        "ES-RA-04_REVIEW_EXECUTION_PROFILE_V1.0.0.json",
        "ES-RA-04_MACHINE_VALIDATION_AGENT.md",
        "ES-PH1-MB04-CANARY-ESRA04-5621e1f7473f489d",
    ),
    "ES-RA-05": (
        "ES-RA-05_REVIEW_EXECUTION_PROFILE_V1.0.0.json",
        "ES-RA-05_EVIDENCE_CUSTODIAN.md",
        "ES-PH1-MB04-CANARY-ESRA05-e8a5406b9e1d4c32",
    ),
}
...
def verify() -> None:
    ...
    for role_id, (profile_name, source_name, own_canary) in ROLES.items():
        ...
        if EXPECTED_STATE == "FROZEN":
            ...
            require(all(path.stat().st_mode & stat.S_IWUSR for path in files), f"non-writable prepared file: {role_id}")
        else:
            raise AssertionError(f"unexpected A04_PACKET_STATE: {EXPECTED_STATE}")

        manifest_path = packet / "INPUT_MANIFEST.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        require(manifest["attempt_id"] == "ES-PH1-PILOT-A-MODE-B-ATTEMPT-04", f"attempt mismatch: {role_id}")
        ...
        require(b"MODE_B_ATTEMPT_04_HIDDEN_DEFECT_ORACLE" not in packet_text, f"oracle filename leak: {role_id}")
        require(b"A04-DEF-" not in packet_text, f"oracle content leak: {role_id}")
    ...
    oracle = oracle_root / "MODE_B_ATTEMPT_04_HIDDEN_DEFECT_ORACLE.csv"
    ...
    if EXPECTED_STATE == "FROZEN":
        require(not (oracle.stat().st_mode & stat.S_IWUSR), "frozen oracle is writable")
    else:
        require(bool(oracle.stat().st_mode & stat.S_IWUSR), "prepared oracle is not writable")
    evidence_names = sorted(path.name for path in evidence_root.iterdir())
    if EXPECTED_STATE == "FROZEN":
        require(evidence_names == ["PACKET_FREEZE_RECEIPT.json"], f"unexpected evidence at frozen verification: {evidence_names}")
    else:
        require(evidence_names == [], "evidence directory not initially empty")
    ...
    print(json.dumps({
        ...
        "packet_state": EXPECTED_STATE,
        "packet_results": packet_results,
        "oracle_sha256": sha256(oracle),
        "evidence_state": evidence_names,
    }, indent=2))
