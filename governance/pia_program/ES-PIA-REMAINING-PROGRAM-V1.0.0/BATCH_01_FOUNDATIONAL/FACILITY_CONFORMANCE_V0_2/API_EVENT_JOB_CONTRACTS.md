...
## V1.0.1 onboarding and context contracts

+Design events include `onboarding.purpose_recorded`, `onboarding.horse_first_completed`, `facility.association_proposed`, `organization.association_proposed`, and `context.switch_confirmed`. Every event carries Tenant, optional Facility/Organization, principal, actor chain, effective time, purpose, provenance, context version, idempotency key, and `authority_conferred=false` unless a separate Permission-owned event supplies authority. Consumers must accept null Facility/Organization identifiers for horse-first onboarding and must fail closed on stale or mismatched context.
