...
Template name: `REOPENING_NOTICE`.

Template version: `V4_PURPOSE_SPECIFIC_DRAFT`.

## Purpose

Documents when a previously retained or remediated item must be reopened because evidence, source bytes, or reviewer conclusions changed.

## Required Evidence Fields

`notice_id, reopened_finding, reopening_trigger, evidence_changed, affected_package_version, required_action, owner, due_date`
...
|---|---|---|---|---|
| notice_id | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
| due_date | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
