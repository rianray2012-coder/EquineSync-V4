...
Binds each cited source row to immutable bytes and declares whether it is controlling, candidate, duplicate, superseded, or excluded.

## Required Evidence Fields

`source_id, repository_path, source_title, sha256, byte_length, git_blob, controlling_version, custody_state`
...
|---|---|---|---|---|
| source_id | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
| custody_state | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
