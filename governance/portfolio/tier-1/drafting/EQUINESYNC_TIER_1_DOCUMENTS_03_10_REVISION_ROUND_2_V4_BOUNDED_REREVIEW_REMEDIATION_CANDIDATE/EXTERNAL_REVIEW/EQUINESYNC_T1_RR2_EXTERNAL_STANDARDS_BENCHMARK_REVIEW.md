...
**F-20 · Adoption-evidence labels conflict with the package-wide `NOT_ADOPTED` state.** `08_.../SOURCE_AUTHORITY_DISPOSITION_REGISTER.csv`, `authority_state`: 598 rows `ADOPTION_OR_LOCK_EVIDENCE_PRESENT`, 170 rows `SOURCE_CONTAINS_FOUNDER_APPROVAL_EVIDENCE_NOT_PACKAGE_ADOPTION`.
...
> Do you approve the thirteen-state lifecycle vocabulary and the state-transition rules defined in `04_AUTHORITY_LIFECYCLE_REGISTER/` as the **documentary vocabulary** to be used in future EquineSync Tier 1 governance drafting?
