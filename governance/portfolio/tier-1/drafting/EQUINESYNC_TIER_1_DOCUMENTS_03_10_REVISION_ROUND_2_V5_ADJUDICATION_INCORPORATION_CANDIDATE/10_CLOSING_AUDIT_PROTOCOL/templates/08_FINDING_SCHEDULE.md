...
Template name: `FINDING_SCHEDULE`.

Template version: `V4_PURPOSE_SPECIFIC_DRAFT`.

## Purpose

Maps each finding to evidence, remediation, and retained-open status using row-level identifiers.

## Required Evidence Fields

`finding_id, source_review_id, severity, observed_fact, affected_document, required_remediation, closure_evidence, closure_status`
...
|---|---|---|---|---|
| finding_id | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
| closure_status | REQUIRED_BEFORE_USE | REQUIRED_BEFORE_USE | OPEN_PENDING_COMPLETION | V4 template only; not completed evidence |
