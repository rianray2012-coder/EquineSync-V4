...
This agrees with the package's own self-declared status (`REVISION_REQUIRED_PACKAGING_READY_FOR_BOUNDED_REREVIEW_ACCEPTED_CONTENT_REVISION_REQUIRED`), but for a materially different and more serious reason than the package records: **V2 introduces a regression that breaks the package integrity chain**, and the new V2 validator does not detect it.
...
3. **Honest self-declared disposition.** `PACKAGE_STATUS = REVISION_REQUIRED_PACKAGING_READY_FOR_BOUNDED_REREVIEW_ACCEPTED_CONTENT_REVISION_REQUIRED`, and the validator emits `STRUCTURAL_PASS_SUBSTANTIVE_CONTROL_BLOCKED` rather than a bare `PASS`. Separating structural conformance from substantive closure is conceptually the right control.
