...
**Version:** 2.0  
**Status:** Corrected Version 2.0 Candidate Following Founder `ACCEPT_WITH_MODIFICATION`  
**Product:** EquineSync  
...
The architecture may identify illustrative implementation candidates while preserving a replaceable adapter boundary and a governed vendor-exit path.
...
## 2.2 Illustrative provider candidates, replaceable adapter

EquineSync adopts the strategy:

> Evaluate illustrative provider candidates, preserve a vendor-neutral contract, and require a documented replacement path.

Every provider named anywhere in this model, including Stripe, DocuSign, Google, Microsoft, Twilio, SendGrid, Cloudflare, Render, Vercel, Supabase, and future providers, is an illustrative implementation candidate only. Inclusion does not imply constitutional approval, endorsement, preferred status, procurement authority, mandatory adoption, credential authority, implementation authority, or activation authority. Provider selection requires a separate founder decision and applicable readiness, security, privacy, legal, financial, implementation, and environment gates. No provider name may become the only representation of a domain capability.
...
**Illustrative candidate:** Vercel

Vercel illustrates the currently observed frontend-hosting implementation and is not constitutionally approved or required by this model.
...
**Illustrative candidate:** Render

Render illustrates the currently observed backend-hosting implementation and is not constitutionally approved or required by this model.
...
## 7.2 Illustrative provider candidates

- For MongoDB evaluation, MongoDB Atlas is an illustrative managed-provider candidate.
- For PostgreSQL evaluation, Supabase PostgreSQL and Render PostgreSQL are illustrative managed-provider candidates.
...
## 8.1 Illustrative provider candidates

**Object-storage candidate:** Cloudflare R2  
...
- other provider candidates evaluated through a separately authorized process.
...
## 9.4 Illustrative evaluation candidates

Clerk may be evaluated for polished implementation and developer experience.

Auth0 may be evaluated if enterprise SSO and federation become an immediate requirement.
...
## 10.1 Illustrative provider candidate

**Resend**

## 10.2 Illustrative capability scope
...
## 11.1 Illustrative provider candidate
...
## 12.1 Illustrative provider candidate

**Twilio**

## 12.2 Illustrative capability scope
...
## 14.1 Illustrative provider candidates

- Google Calendar: full adapter;
- Microsoft Outlook / Microsoft Graph: full adapter;
- Apple Calendar: illustrative ICS adapter candidate;
...
## 15.1 Illustrative provider candidate
...
## 16.1 Illustrative provider candidate
...
Stripe Connect embedded or Express-style onboarding may be evaluated for barns and providers through a separately authorized payments decision.
...
PayPal Checkout with Venmo is an illustrative payment-adapter candidate subject to separate provider selection and implementation governance.

## 17.4 Cash App Pay

Cash App Pay through Stripe is an illustrative option subject to a separately authorized payments decision.
...
## 18.1 Illustrative provider candidate

**QuickBooks Online**

## 18.2 Illustrative alternative candidate

**Xero** is an illustrative accounting-adapter candidate subject to future provider evaluation.
...
- an error and performance provider selected through a future authorized process, with Sentry as an illustrative candidate;
...
## 20.1 Illustrative provider candidate

**Freshdesk**

## 20.2 Future option

Intercom is an illustrative support-platform candidate for capabilities such as:
...
## 21.1 Illustrative provider candidate
...
## 22.2 Illustrative provider candidate

OpenAI is an illustrative adapter candidate subject to RF30 boundaries and a separately authorized AI phase.
...
## 23.1 Illustrative provider candidates
...
## 41.1 Illustrative implementation-candidate set

The following names are examples for future evaluation only. They are not constitutionally approved, endorsed, preferred, mandatory, selected, procured, configured, or authorized for implementation or activation by this model.
...
Calendar adapter candidates: Google, Microsoft, Apple, and standards-based ICS providers

E-signature candidates: DocuSign, Adobe Sign, Dropbox Sign, native low-risk acknowledgment

Payment-adapter candidates: Stripe Billing, Payments, Connect, Cash App Pay, PayPal Checkout, Venmo
Manual recording: Zelle, cash, check, wallet transfers

Accounting-adapter candidates: QuickBooks Online and Xero

Observability candidates: OpenTelemetry-compatible tooling and Sentry
Support-platform candidates: Freshdesk and Intercom
Analytics: PostHog or equivalent
AI: provider-neutral; OpenAI is an illustrative candidate only
Mapping candidates: Google Maps, Apple Maps, and Mapbox
...
Vendor strategy: separately selected provider, replaceable adapter
```
...
- disputes;
- Cash App Pay;
- additional payment-adapter capability, with PayPal and Venmo as illustrative candidates;
...
2. capability-first, provider-neutral adapter strategy;
3. provider-evaluation criteria and separate selection process;
4. database capability boundary and separately governed provider selection;
5. provider-neutral storage capability and compatibility posture;
6. identity evaluation path;
7. transactional and marketing separation;
8. messaging-adapter capability and separately gated activation scope;
9. push architecture;
10. calendar adapter scope;
11. e-signature adapter capability and provider-selection criteria;
12. payment and connected-account adapter capability;
13. merchant and connected-account posture;
14. accounting-adapter capability and provider-selection criteria;
15. additional payment-adapter evaluation criteria;
16. observability and support capability requirements;
17. analytics privacy and adapter boundary;
18. provider-neutral AI candidate-evaluation criteria under RF30;
19. mapping capability and platform-boundary requirements;
20. geospatial-adapter evaluation criteria;
...
- provider categories are bounded, providers remain replaceable and subordinate, and future provider selection is separately gated;
...
Version 1.0 is historically unavailable, so complete predecessor preservation cannot be verified. Available repository evidence supports a strong provider-neutral architecture, and Version 2.0 deepens several operational areas before any future lock decision.
...
These additions preserve the provider-neutral principle evidenced in available repository sources: external providers are adapters rather than domain authorities. This is not a claim of complete Version 1 preservation.
...
- available predecessor principles are mapped through the Historical Provenance Exception Ledger without claiming complete Version 1 preservation;
