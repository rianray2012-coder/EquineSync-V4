...
**Candidate Version:** 1.2
**Status:** Controlled Revised Candidate; Founder Approval and Adoption Required
...
16. A user-supplied or provider-generated URL is an untrusted reference, not a governed asset.
17. Upload intent, byte transfer, quarantine acceptance, scan approval, canonical commit, and publication are separate states.
18. Harmful or unlawful content requires safety-preserving restriction and specialist handling, not silent deletion or ordinary moderator access.
19. Integrity evidence must identify its algorithm, input, time source, and scope; it must not overstate authenticity.
20. Communication delivery never expands attachment authority.
...
## 4.9 External asset reference

An `External Asset Reference` identifies content outside an EquineSync-controlled asset boundary. It is untrusted, potentially mutable, independently permissioned, and capable of leaking recipient or request metadata. It must never be represented as a stored, scanned, durable, private, or canonical EquineSync asset unless a governed ingest completes.

## 4.10 Upload transaction

An `Upload Transaction` is a bounded, actor- and environment-scoped process from intent through byte receipt, verification, quarantine, processing, canonical commit, and cleanup. A transfer response alone does not prove that the intended bytes were durably and safely accepted.

## 4.11 Delivery projection

A `Delivery Projection` is a recipient-, channel-, purpose-, and time-bound representation used in a message, notification, email, export, webhook, or other delivery channel. It has separate authorization, minimization, expiry, and audit requirements.
...
checksum_scope
received_byte_size
ingest_transaction_id
ingest_state
quarantine_object_reference
...
## 8.1 Upload finalization and transactional integrity

Upload finalization must verify the exact transaction, actor, tenant, barn, intended parent record, object identity, expected revision, expiry, received byte count, verified media type, checksum, quarantine state, and storage-provider result before creating an active canonical asset reference.
...
Failure at any stage must remain explicit. The interface must not show success before the responsible durable boundary accepts the bytes and the canonical commit. A record must not point to an unverified public URL merely because a client-side upload returned success.

Abandoned, expired, failed, duplicated, or mismatched uploads require bounded orphan cleanup that respects evidence, incident, hold, and retry policy. Finalization and cleanup must be idempotent and must never delete another transaction's object.

# 9. Quarantine, Validation, and Malware Safety

New or externally sourced assets must enter an appropriate quarantine boundary before any processing or ordinary use.
...
Unsafe, unknown, timed-out, or failed scanning remains quarantined or fails closed. When scanning is implemented, high-risk classes should support multiple independent malware or validation engines according to governed risk policy. Encrypted files require an explicitly justified workflow and must remain unavailable until safely validated. Scripts, macros, and other active content must never execute within EquineSync. Scan-provider output is evidence, not infallible truth. Originals needed for investigation must be protected from ordinary access.

## 9.1 Harmful, unlawful, and safeguarding-sensitive content

Content that may involve child sexual abuse material, non-consensual intimate imagery, credible threats, harassment, stalking, prohibited contact, animal abuse or neglect, fraud, identity-document misuse, malicious code, or other unlawful or high-risk material must enter a specialist restriction path.

The path must define:

- immediate access restriction and least-privilege responder access;
- preservation that does not create unnecessary copies or expose ordinary staff;
- applicable reporting, escalation, legal, safeguarding, and emergency procedures;
- reviewer safety, access logging, and need-to-know handling;
- separation of automated signals from verified determinations;
- claim, challenge, correction, and false-positive handling;
- retention, legal hold, lawful deletion, and evidence-minimization decisions;
- prohibition on re-publication, unsafe preview generation, ordinary AI processing, or broad search indexing.

Automated detection may restrict or escalate content but must not silently determine criminality, abuse, ownership, consent, or permanent destruction. Where immediate human or animal safety is at risk, controlling emergency and safeguarding policy governs.

## 9.2 Continuous safety review

Previously accepted assets may require governed re-scan or reclassification when threat intelligence, parser safety, rights, consent, subject identity, prohibited-contact status, or policy changes. Re-review must preserve prior decisions, avoid retroactive truth claims, and reconcile affected renditions, links, indexes, and publications.
...
The governed base asset inherits the strictest applicable classification and may become more restrictive, but it must not be silently relaxed. A separately identified, purpose-bound, reviewed rendition may receive a different projection classification only when its content and metadata genuinely support that decision; it does not reduce the source asset's classification.
...
Biometric templates, face embeddings, gait signatures, voiceprints, or equivalent recognition features are separate restricted derived records. Their creation, purpose, consent or other authority, retention, sharing, challenge, deletion, and provider processing require explicit governance. Ordinary photo consent does not imply biometric-analysis consent, and recognition output must not independently establish identity or authority.
...
Public distribution requires evidence of ownership, license, consent, statutory exception, or other applicable authority. The system must support attribution requirements, copyright claims, takedown requests, disputed-ownership restrictions, and preservation of the evidence and decisions involved without adjudicating legal ownership.
...
Consent must be purpose-specific, audience-specific, versioned, and revocable where legally permissible. At the age of majority, authority must transition through governed identity verification while preserving historical consent evidence and without silently restoring or expanding access.
...
The original asset and its evidentiary metadata must be preserved exactly where chain of custody, legal, financial, medical, historical, or dispute obligations require it. Metadata removal or suppression applies to audience-specific renditions and external projections, not by silently altering the governed original. Public-sharing policy must support configurable stripping or generalization of precise geolocation.
...
OCR text, transcripts, captions, labels, AI tags, and structured extraction are derived records. They must retain source version, processor, extraction provenance, confidence score, language, review status, and correction history. They remain challengeable and never replace the authoritative original.
...
AI-generated or materially AI-modified assets must be explicitly labeled and disclose their origin. They must preserve a governed prompt reference and only the minimum raw prompt content needed for provenance, model and version, generation time, source inputs, permissions, human reviewer, approval status, disclosure state, and derivative lineage where policy requires. Raw prompts may contain secrets, medical details, minor information, private instructions, or third-party content and must not be retained by default merely to improve provenance. AI assets may never silently replace originals.
...
Downloads, previews, exports, CDN objects, thumbnails, cached copies, and temporary URLs must apply least privilege, bounded expiry, revocation, watermarking where appropriate, and audit logging. Revoked authority must prevent future access immediately; copies already exported remain governed through notice, contractual, retention, and incident controls rather than false technical-recall claims.

## 20.1 Remote URLs and external retrieval

Raw `photo_url`, `document_url`, `file_url`, public-object URL, or similar string fields must be treated as legacy or external references, not canonical asset identity. User-facing clients must not assume such URLs are private, stable, scanned, available, same-tenant, or safe.

Any server-side retrieval of an external reference requires a separately authorized ingestion path with scheme and destination policy, DNS and redirect revalidation, private-network and metadata-service denial, response-size and timeout limits, content-type verification, decompression limits, credential isolation, egress policy, quarantine, and audit. This is required to prevent server-side request forgery and unintended network access.

Client-side display of remote media must consider tracking pixels, referrer and IP disclosure, cookies, mixed content, content mutation, malicious redirects, hotlinking, availability, consent, and accessibility. High-sensitivity records must use approved EquineSync-controlled projections rather than uncontrolled remote embeds.

Migration from raw URLs to canonical asset references must be additive and evidence-preserving. It must classify unreachable, mutable, cross-tenant, public, provider-owned, duplicated, unsafe, and rights-unknown references without guessing or silently fetching production content.
...
## 22.1 Communication, notification, and notice attachments

Message composition, recipient eligibility, attachment eligibility, channel delivery, and attachment opening are separate decisions. A sender's access does not prove that every recipient may receive or open the asset.

Each delivery projection must bind the exact asset version or approved rendition to the recipient or governed audience, channel, purpose, classification, expiry, notification-preview policy, and audit correlation. Email, SMS, push, calendar, webhook, and lock-screen content must not contain unrestricted originals, reusable public URLs, secrets, sensitive filenames, or revealing thumbnails.

Authorization must be re-evaluated when a recipient opens an in-platform attachment. Where an external delivery necessarily creates a recipient-controlled copy, the system must disclose that revocation cannot reliably recall it and apply the minimum approved rendition.
...
Syndication to websites, social networks, marketplaces, press systems, public CDNs, search engines, or other channels creates separately governed downstream copies. Publication records must identify destinations, identifiers, rendition versions, terms, removal capability, known caches, and withdrawal outcomes. EquineSync must not claim complete recall from systems it cannot control.
...
- the exact signed artifact without signature migration to an altered document version;
- signature metadata, signer identity, timestamp, jurisdictional evidence, and cryptographic verification where available;
...
Brand assets require canonical source identity, approved variant, rights, version, palette and use rules, minimum size, accessibility, allowed surfaces, and retirement state. Their governed lifecycle must distinguish `CONCEPT`, `INTERNAL_REVIEW`, `APPROVED_BRAND_STANDARD`, `PRODUCTION_APPROVED`, `DEPRECATED`, `RETIRED`, and `HISTORICAL_ARCHIVE`. Approved usage rights and variants must be centrally governed.
...
Retention is governed exclusively by the controlling Master Record Stewardship and Retention Model and is determined through record class, subject, author/steward, purpose, agreement, legal requirement, claim, incident, and environment. Storage lifecycle defaults must not silently decide legal retention.
...
Deletion must be idempotent, observable, retryable, and exception-ledgered. Legal holds supersede incompatible deletion. Purge completion cannot be claimed until required primary storage, backups, caches, replicas, search indexes, derivatives, and controllable exported projections reconcile, subject to the Master Record Stewardship and Retention Model and explicit technical-feasibility evidence.
...
## 32.1 Cryptographic integrity and time evidence

Every integrity value must identify the approved algorithm, encoded input scope, canonicalization or byte-exact rule, generation time, generating actor or service, and verification result. Hash agility must allow a stronger governed algorithm to be added without rewriting or invalidating historical evidence.

Checksum collision, algorithm deprecation, signature expiry, certificate revocation, clock uncertainty, and missing trusted-time evidence require explicit status rather than silent success. Capture time, client time, server receipt time, provider time, processing time, and verified trusted time must remain distinct.

Cryptographic hashes, signatures, manifests, content credentials, and trusted timestamps may strengthen integrity and provenance evidence. They do not independently prove authorship, consent, legality, factual truth, identity, or admissibility.
...
Production-like environments must fail closed when required storage, scanning, encryption, signing, or processing configuration is absent or invalid. They must not silently fall back to a local stub, public object, insecure bucket, or non-durable implementation while presenting successful storage. Development stubs must be unmistakable, environment-bound, incapable of production activation, and excluded from durability claims.
...
## 34.1 Live capture, streaming, and continuous recording

Live video, audio, camera feeds, body-worn capture, arena streams, remote veterinary sessions, and continuous recording require separate authorization from ordinary file upload. Governance must address visible notice, subject and bystander consent, minors, precise location, recording indicators, stream access, recording retention, clipping, screenshots, moderation, emergency access, latency, interruption, and downstream recording.

Live access must be short-lived, audience-bound, and non-discoverable by default. Streaming authorization does not automatically authorize recording, replay, download, AI analysis, biometric processing, public broadcast, or creation of permanent evidence.
...
Capacity, quota, and cost controls must account for actor, tenant, barn, asset class, storage tier, processing intensity, derivative fan-out, bandwidth, retry volume, and provider cost. Limits must prevent denial of service and billing abuse without silently discarding safety, medical, legal-hold, or evidentiary content. Rejection, deferral, degraded processing, and administrative override require explicit status, reason, authority, and audit evidence.

# 38A. Additional Constitutional Media Controls

## 38A.1 Authenticity and tamper detection

Authenticity controls must distinguish content integrity from factual truth. Governed hashes, signatures, capture provenance, transformation history, anomaly signals, and reviewer decisions may identify alteration or inconsistency, but no technical signal alone proves an event occurred or that an asset is lawful. Evidentiary originals must remain preserved while suspected manipulation is reviewed.

## 38A.2 Duplicate detection and deduplication

Duplicate and near-duplicate detection may reduce redundant storage or identify related submissions only through governed policy. Independent submissions, custody events, uploader context, rights, classification, retention, and evidence lineage must remain distinct even when binary content is shared. Deduplication must never merge authority, delete evidence, broaden access, or conceal a separate submission.

## 38A.3 Large and specialized media processing

Video, drone footage, diagnostic imaging, long audio, high-resolution scans, and other high-volume assets require explicit size, duration, resource, timeout, resumability, quarantine, processing, storage, cost, retention, and degraded-state rules. Partial processing must remain visible and may not be represented as complete.

## 38A.4 Sensitive veterinary imaging

Radiographs, ultrasound, endoscopy, pathology images, thermal images, laboratory media, and related clinical assets are restricted medical records unless controlling policy classifies a specific projection otherwise. Their metadata, annotations, measurements, professional interpretations, provider authorship, subject identity, and transfer behavior require medical permission and source-professional boundaries.

## 38A.5 Forensic chain of custody

Investigations, disputes, safeguarding, insurance, litigation, claims, and other evidentiary uses require acquisition source, custodian sequence, immutable version, checksum, access, transfer, transformation, exception, hold, and receiving-party evidence. The platform records custody and technical integrity; it does not claim legal admissibility.

## 38A.6 Watermarking and authenticity seals

Visible watermarks, invisible marks, authenticity seals, and provenance labels are governed renditions or metadata. They must identify scope, issuer, purpose, version, placement, durability, verification method, and removal authority. A watermark or seal does not create ownership, consent, truth, or permission and must not obscure required clinical or accessibility information.

## 38A.7 Bulk import and migration

Bulk import and migration must preserve stable import identity, source system, source identifier, source path as attributed evidence, checksum, provenance, rights, classifications, relationships, retention, duplicate candidates, exceptions, and access-delta results. Historical media may not be promoted to verified authority from filenames, folders, or legacy possession alone.

## 38A.8 Offline media synchronization

Offline media flows require actor-, barn-, device-, and session-scoped queues; deterministic ordering; immutable local request identity; checksums before and after transfer; conflict classification; retry and reconciliation; logout purge or protected retention according to policy; and explicit local-only, queued, uploading, processing, failed, conflicted, and synchronized states. Offline operation must not broaden access or conceal persistence failure.

## 38A.9 Media quality standards

Quality policy may define minimum resolution, orientation, duration, readability, completeness, corruption checks, checksum validation, color or diagnostic fidelity, accessibility, and fit-for-purpose thresholds by asset type. A low-quality asset may remain valid evidence and must not be deleted or altered merely because it fails a publication or processing quality threshold.

## 38A.10 Format longevity and obsolescence

Long-term preservation requires a controlled format registry, readability monitoring, preservation masters, migration recipes, before-and-after checksums, fidelity review, retained originals, derivative lineage, and rollback or exception evidence. Format conversion must never silently replace an evidentiary original or claim perfect fidelity without validation.
...
- authenticity and tamper-signal handling without truth overclaim;
- duplicate storage optimization without merging independent submissions;
- large-media limits, resumability, timeout, and degraded states;
- veterinary-image medical projection and source-professional restrictions;
- forensic chain-of-custody continuity;
- watermark and authenticity-seal scope and removal controls;
- bulk-import provenance and access-delta reconciliation;
- offline ordering, checksums, conflicts, logout, and persistence failure;
- media-quality thresholds without evidence destruction;
- preservation-format migration fidelity and retained originals;
- disaster-recovery restoration of identity, lineage, classification, permissions, holds, and deletion state;
- periodic integrity audits across canonical metadata, stored objects, derivatives, and governed replicas;
- exact upload-finalization binding, mismatch rejection, orphan cleanup, and no-success-before-commit behavior;
- external URL scheme, redirect, DNS, private-network, size, timeout, mutation, and tracking controls;
- safeguarding restriction, specialist access, false-positive challenge, and evidence-minimization behavior;
- recipient- and channel-specific attachment projections and open-time authorization;
- hash algorithm agility, canonicalization, collision response, and distinct time-source evidence;
- biometric derivation denial without explicit authority and deletion/revocation handling;
- live-stream audience binding and separation of view, recording, replay, download, analysis, and publication;
- storage, bandwidth, derivative, retry, and processing-quota abuse without unsafe evidence loss;
- downstream syndication, withdrawal reconciliation, and honest non-recall claims;
- production configuration failure proving no silent stub, public-object, or non-durable fallback.
...
- file-format risk tiers and active-content policy;
- authenticity and tamper-signal types;
- duplicate and deduplication decisions;
- veterinary-imaging classes;
- custody events and evidence-transfer outcomes;
- watermark and authenticity-seal types;
- media quality profiles;
- preservation formats and obsolescence status;
- external-reference types and retrieval policy;
- upload transaction and finalization outcomes;
- harmful-content and safeguarding restriction classes;
- communication attachment and delivery-projection types;
- checksum algorithms, canonicalization scopes, signatures, and trusted-time evidence;
- biometric and recognition-derived records;
- live-capture and streaming states;
- capacity, quota, cost, and safety-preserving override policy;
- syndication destinations and withdrawal outcomes;
- environment-specific storage and processing activation states;
...
16. Raw or public URLs never become canonical asset identity and must not imply privacy, durability, scanning, or safety.
17. No asset becomes active before exact-byte, scope, quarantine, and durable-commit verification.
18. Harmful-content response protects people, horses, evidence, and responder privacy without automated legal conclusions.
19. Communication and notification channels never broaden attachment permission.
20. Integrity metadata identifies algorithm, scope, and time source and never proves factual truth by itself.
21. Biometric or recognition-derived data requires separate explicit governance.
22. Live viewing never implies recording, replay, analysis, download, or publication.
23. Capacity controls never silently discard safety-critical, held, or evidentiary content.
24. Downstream publication never supports a false complete-recall claim.
25. Production-like environments never silently fall back to non-durable or public storage.
...
21. authenticity and tamper-detection boundaries;
22. duplicate detection and deduplication without evidentiary collapse;
23. large and specialized media processing;
24. sensitive veterinary imaging protections;
25. forensic chain-of-custody requirements;
26. watermarking and authenticity-seal policy;
27. bulk import and migration provenance;
28. offline media synchronization and reconciliation;
29. media quality standards;
30. long-term format preservation and obsolescence policy;
31. raw URL, external-reference, remote-display, and server-side retrieval policy;
32. upload finalization, exact-byte verification, canonical commit, and orphan cleanup;
33. harmful, unlawful, safeguarding-sensitive, and specialist-response policy;
34. communication, notification, notice, calendar, webhook, and email attachment delivery;
35. cryptographic integrity, algorithm agility, canonicalization, and trusted-time evidence;
36. biometric, face, voice, gait, and recognition-derived records;
37. live capture, streaming, recording, replay, screenshot, and continuous-monitoring boundaries;
38. storage, bandwidth, processing, derivative, retry, quota, and cost-abuse controls;
39. downstream syndication, withdrawal, caching, and non-recall truth;
40. production configuration, provider-readiness, and fail-closed fallback behavior.

# 43. Adoption and Implementation Gates

## 43.1 Current repository drift requiring later implementation governance

The current repository contains legacy and readiness behavior that is not evidence of conformance with this candidate:

- horse, rider, provider, veterinary, task, and health-document records may store raw `photo_url`, `document_url`, `file_url`, or related URL strings;
- user interfaces may render or open those references directly;
- the storage abstraction can produce a `public_url` and describes a public media base;
- document-scan readiness creates an upload intent from client-declared MIME type and byte size, then records a URL after client-side transfer without a complete server-side finalization, quarantine, checksum, and scan contract;
- production-provider initialization failure can fall back to a local development stub;
- Passport media composition projects profile and horse-media URL records rather than a fully canonical asset envelope.

These are inventory findings, not implementation authorization and not a declaration that current data is unsafe. A future governed implementation phase must classify the actual environment and data, prevent new drift, add compatibility projections, migrate additively, preserve lineage, and prove no access expansion before deprecating legacy fields.
...
- remote URL ingestion, crawling, preview fetching, streaming, biometric processing, moderation-provider activation, or harmful-content adjudication;
...
`MASTER_MEDIA_FILES_AND_DIGITAL_ASSET_GOVERNANCE_MODEL_V1_2_READY_FOR_FOUNDER_APPROVAL_REVIEW`
