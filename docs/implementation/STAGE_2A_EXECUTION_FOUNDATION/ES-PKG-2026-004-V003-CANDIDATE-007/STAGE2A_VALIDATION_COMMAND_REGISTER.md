...
- `S2A-CMD-004` `stage2a/.venv/bin/python -m unittest discover -s stage2a/tests -p test_*.py` — `PASS_36_OF_36`
- `S2A-CMD-005` `sh stage2a/run_validation.sh` — `PASS_16_OF_16`
- `S2A-CMD-006` `stage2a/.venv/bin/python PACKAGED_VALIDATE_STAGE2A_PACKAGE PACKAGE --phase assembly` — `PASS_ASSEMBLY_4_OF_4_INCLUDING_DETACHED`
- `S2A-CMD-007` `PACKAGED_PYTHON validate_stage2a_package.py DETACHED_PACKAGE --phase assembly` — `PASS_DETACHED_PACKAGE_SOURCE_PAYLOAD`
